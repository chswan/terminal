...
try
{
    ...
    const auto needToWriteFile = settingsString.empty();
    std::vector<Model::SettingsLoadWarnings> warnings;

    auto inboxSettings = _parse(OriginTag::InBox, DefaultJson, warnings);
    ParsedSettings userSettings;

    try
    {
        userSettings = _parse(OriginTag::User, settingsStringView, warnings);
    }
    ...
    const auto getDynamicProfiles = [&]() {
        ...
        return gsl::span{ data + userProfileCount, data + size };
    };
    ...
    _generateProfiles(ignoredNamespaces, inboxSettings.profiles);

    if (settingsString.empty())
    {
        _fillBlanksInDefaultsJson(inboxSettings.profiles, userSettings);
    }

    _layerGeneratedProfiles(inboxSettings.profiles, userSettings);
    // _layerFragments occurs after _layerGeneratedProfiles as fragments
    // might reference profiles generated by a generator.
    _layerFragments(ignoredNamespaces, userSettings, warnings);
    _disableDeletedProfiles(getDynamicProfiles());
    ...
    const auto settings = winrt::make_self<CascadiaSettings>();
    {
        ...
        settings->_userDefaultProfileSettings = userSettings.baseLayerProfile;
...
winrt::Microsoft::Terminal::Settings::Model::CascadiaSettings CascadiaSettings::LoadUniversal()
{
    const auto settings{ winrt::make_self<CascadiaSettings>() };
    std::vector<Model::SettingsLoadWarnings> warnings;
    const auto parsed = _parse(OriginTag::InBox, DefaultUniversalJson, warnings);
...
winrt::Microsoft::Terminal::Settings::Model::CascadiaSettings CascadiaSettings::LoadDefaults()
{
    const auto settings{ winrt::make_self<CascadiaSettings>() };
    std::vector<Model::SettingsLoadWarnings> warnings;
    const auto parsed = _parse(OriginTag::InBox, DefaultJson, warnings);
    settings->_globals = parsed.globals;
    settings->_allProfiles = parsed.foobar();
    settings->_finalizeSettings();
    return *settings;
...
ParsedSettings CascadiaSettings::_parse(const OriginTag origin, const std::string_view& content, std::vector<Model::SettingsLoadWarnings>& warnings)
{
    ParsedSettings settings;
    _parse(origin, content, warnings, settings);
    return settings;
}

void CascadiaSettings::_parse(const OriginTag origin, const std::string_view& content, std::vector<Model::SettingsLoadWarnings>& warnings, ParsedSettings& settings)
{
    ...
    // profiles.list
    {
        ...
        for (const auto& profileJson : profilesArray)
        {
            if (_isValidProfileObject(profileJson))
            {
                ...
                _append(profile, warnings, settings);
            }
        }
    }
}

void CascadiaSettings::_append(const winrt::com_ptr<Profile>& profile, std::vector<Model::SettingsLoadWarnings>& warnings, ParsedSettings& settings)
{
    ...
    {
        warnings.emplace_back(SettingsLoadWarnings::DuplicateProfile);
    }
}
...
winrt::com_ptr<Profile> CascadiaSettings::_reproduceProfile(const winrt::com_ptr<Profile>& parent)
{
...
void CascadiaSettings::_generateProfiles(const std::unordered_set<std::wstring_view>& ignoredNamespaces, std::vector<winrt::com_ptr<Profile>>& generatedProfiles)
{
...
void CascadiaSettings::_layerGeneratedProfiles(const std::vector<winrt::com_ptr<Profile>>& generatedProfiles, ParsedSettings& userSettings)
{
    for (const auto& generatedProfile : generatedProfiles)
    {
        ...
        {
            userSettings.profiles.emplace_back(_reproduceProfile(generatedProfile));
        }
    }
}
...
void CascadiaSettings::_fillBlanksInDefaultsJson(const std::vector<winrt::com_ptr<Profile>>& generatedProfiles, const ParsedSettings& userSettings)
{
    // 1.
    {
...
void CascadiaSettings::_layerFragments(const std::unordered_set<std::wstring_view>& ignoredNamespaces, ParsedSettings& userSettings, std::vector<Model::SettingsLoadWarnings>& warnings)
{
        ...
        for (const auto& fragmentExt : std::filesystem::directory_iterator(path))
        {
            if (fragmentExt.path().extension() == jsonExtension)
            {
                try
                {
                    ...
                    _parse(OriginTag::Fragment, content, warnings, fragmentSettings);
                    ...
                    for (const auto& fragmentProfile : fragmentSettings.profiles)
                    {
                        ...
                        else
                        {
                            ...
                            fragmentProfile->Source(source);
                            _append(_reproduceProfile(fragmentProfile), warnings, userSettings);
                        }
                    }
...
void CascadiaSettings::_disableDeletedProfiles(const gsl::span<winrt::com_ptr<implementation::Profile>>& dynamicProfiles)
{
    const auto& state = winrt::get_self<ApplicationState>(Model::ApplicationState::SharedInstance());
    auto generatedProfileIds = state->GeneratedProfiles();
    bool newGeneratedProfiles = false;

    for (const auto& profile : dynamicProfiles)
    {
...
void CascadiaSettings::_finalizeSettings() const
{
    ...
    {
        _globals->DefaultProfile(_getProfileGuidByName(unparsedDefaultProfile).value_or(winrt::guid{}));
    }
}
