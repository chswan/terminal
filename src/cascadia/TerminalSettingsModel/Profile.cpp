...
winrt::Microsoft::Terminal::Settings::Model::Profile Profile::Copy()
{
    return *CopySettings();
}

winrt::com_ptr<Profile> Profile::CopySettings()
{
    ...
    sourceFontInfoImpl.copy_from(winrt::get_self<FontConfig>(_FontInfo));
    ...
    sourceDefaultAppearanceImpl.copy_from(winrt::get_self<AppearanceConfig>(_DefaultAppearance));
    ...
    if (_UnfocusedAppearance.has_value())
    {
        Model::AppearanceConfig unfocused{ nullptr };
        if (_UnfocusedAppearance.value() != nullptr)
        {
            // Copy over the unfocused appearance
            winrt::com_ptr<AppearanceConfig> sourceUnfocusedAppearanceImpl;
            sourceUnfocusedAppearanceImpl.copy_from(winrt::get_self<AppearanceConfig>(_UnfocusedAppearance.value()));
...
winrt::com_ptr<Profile> Profile::CloneInheritanceGraph(winrt::com_ptr<Profile> sourceGraph, winrt::com_ptr<Profile> cloneGraph, std::unordered_map<void*, winrt::com_ptr<Profile>>& visited)
{
    ...
    //   and we have parents...
    if (!visited.count(sourceGraph.get()) && !sourceGraph->_parents.empty())
    {
