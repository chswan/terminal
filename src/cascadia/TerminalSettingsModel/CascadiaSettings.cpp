...
void CascadiaSettings::_CopyProfileInheritanceTree(winrt::com_ptr<CascadiaSettings>& cloneSettings) const
{
    ...
    for (const auto& profile : _allProfiles)
    {
        ...
        dummyRootSource->_FinalizeInheritance();
    }
    ...
    if (_userDefaultProfileSettings)
    {
        ...
        cloneSettings->_userDefaultProfileSettings = _userDefaultProfileSettings->CopySettings();
...
winrt::com_ptr<Profile> CascadiaSettings::_createNewProfile(const std::wstring_view& name) const
{
    ...
    const auto profile = _reproduceProfile(_userDefaultProfileSettings);
    profile->Guid(guid);
